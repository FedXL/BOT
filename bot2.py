...
try:
    ...
    eur ="Сегодня без курса"
    use ="Сегодня без курса"
...
async def welcome_message(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, f"Здравcтсвуйте {md.bold(message.from_user.username)}!"
                                                 f"Я бот помощник Федор.\n"
                                                 f"Я помогу сделать заказ и знаю\n"
                                                 f"ответы на большинство ваших вопросов \n"
                                                 f"Помогу рассчитать примерную стоимость"
                                                 f" заказа, а также могу связать с живым"
                                                 f" консультантом",
...
async def buyout(message: types.Message, state: FSMContext):
...
async def order_kaz(message: types.Message):
    await BotStates.order_kaz_choice.set()
...
@dp.message_handler(Text(equals="Назад"), state=BotStates.order_kaz_choice)
async def back_btn_order_kaz(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("Меню выбора заказа", reply_markup=nv.SuperMenu.invoiceMenu)


@dp.message_handler(state=BotStates.order_kaz_choice)
async def order_kaz_choice(message: types.Message, state: FSMContext):
    if message.text == "Предоставлю доступ в личный кабинет":
        ...
        await BotStates.order_kaz_ch1_shop_name.set()
...
@dp.message_handler(Text(equals="Назад"), state=BotStates.ordder_kaz_ch2_href)
async def back_btn_order_kaz(message: types.Message, state: FSMContext):
    await BotStates.order_kaz_choice.set()
    await message.answer('Вы вернулись назад! Теперь нам нужно получить либо доступ'
                         ' к корзине в магазине, '
...
@dp.message_handler(Text(equals="Назад"), state=BotStates.order_kaz_ch1_loggin)
async def back_btn_order_kaz(message: types.Message, state: FSMContext):
    await state.finish()
    await BotStates.order_kaz_choice.set()
    await message.answer('Вы вернулись назад! Теперь нам нужно получить либо доступ'
...
@dp.message_handler(Text(equals="Назад"),
                    state=BotStates.order_kaz_ch1_password)
async def back_btn_order_kaz(message: types.Message, state: FSMContext):
    await state.finish()
    await BotStates.order_kaz_choice.set()
    await message.answer('Вы вернулись назад! Теперь нам нужно получить либо доступ'
...
@dp.message_handler(state=BotStates.order_kaz_ch1_shop_name)
async def get_shop_name(message: types.Message, state):
    ...
    if not valid:
        ...
        await BotStates.order_kaz_ch1_loggin.set()


@dp.message_handler(state=BotStates.order_kaz_ch1_loggin)
async def get_shop_name(message: types.Message, state):
    if len(message.text) > 25:
        ...
        await BotStates.order_kaz_ch1_password.set()


@dp.message_handler(state=BotStates.order_kaz_ch1_password)
async def get_shop_name(message: types.Message, state):
...
@dp.message_handler(Text(equals='Завершить заказ'),
                    state=BotStates.ordder_kaz_ch2_href)
async def href(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        hrefs = [data.get(key) for key in hrefs_num]
        newlist = [md.link("Товар-" + str(i), hrefs[i]) for i in range(num)]
...
@dp.message_handler(state=BotStates.ordder_kaz_ch2_href)
async def href(message: types.Message, state: FSMContext):
    vaflalist = (
        'первая',
        'вторая',
